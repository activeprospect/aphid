...
Aphid.Model.Base = Aphid.Class.create("Aphid.Model.Base", Aphid.Support.Object, {
  ...
  **/
  ...
  _initializeBelongsToAssociation: function(association)
  {
    ...
    {
      // Setup up a proxy functionLoad the Association
...
Aphid.Model.Base.ClassMethods = {
  ...
  **/
  load: function()
  {
    ...
    if (id)
    {
      $L.info("Loading record with identifier: " + id + " ...", this.className);

      var instancePathTemplate = new Template(modelKlass.prototype.get("instancePath"));
      var instancePath         = instancePathTemplate.evaluate({ identifier: id });

      window.console.log(instancePath)

      url = modelKlass.prototype.get("siteUrl") + instancePath;
    }

    // Load Singleton?
    else
    {
      var instancePath = modelKlass.prototype.get("instancePath");
      url = modelKlass.prototype.get("siteUrl") + instancePath;
    }
    ...
    // var adapter = this.prototype.adapter;
    // 
    // if (!Object.isFunction(adapter.load))
    // {
    //   $L.error("Adapter defined on " + this.prototype.displayName + " does not implement load support!", this);
    //   return false;
    // }
    // 
    // return adapter.load(this, url, options);

    var siteUrl      = this.prototype.get("siteUrl"),
        instancePath = this.prototype.get("instancePath");
